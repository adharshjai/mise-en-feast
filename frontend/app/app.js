...
function analyze(dish) {
  const ings = dish.ingredients.map(i => {
    ...
    return { ...i, item, avail, have: avail > 0 || Boolean(i.staple) };
  });
...
// Takes a deck analysis or a bare dish. The Tonight and Saved lists are the sheets allowed to open it sheet-to-sheet.
function openDetail(a) {
  if (!a || state.leaving) return;
  if (state.sheet && state.sheet !== 'tonight' && state.sheet !== 'saved') return;
  const fromSaved = state.sheet === 'saved';
  a = analyze(a.dish || a);
  const d = a.dish;
...
function openMadeIt(dishId) {
  ...
  // only what the pantry actually tracks can be deducted; an assumed staple has no lot
  const rows = a.have.filter(i => i.item).map(i => {
    const cur = i.avail;
